...
class FixedQueue:
    ...
    class Empty(Exception):
        '''비어 있는 FixedQueue에서 다큐 또는 피크할 때 내보내는 예외 처리'''
        pass

    class Full(Exception):
        '''가득 차 있는 FixedQueue에서 인큐할 때 보내는 예외 처리'''
        pass

    def __init__(self, cacacity: int) -> None:
        '''큐 초기화'''
        self.no = 0                     #현재 데이터 개수
        self.front = 0                  #맨 앞 원소 커서
        self.rear = 0                   #맨 끝 원소 커서
        self.capacity = cacacity        #큐의 크기
        self.que = [None] * cacacity    #큐의 본체
    ...
    def count(self, value: Any) -> bool:
        ...
        for i in range(self.no):        # 큐 데이터를 선형 검색
            idx = (i + self.front) % self.capacity
            if self.que[idx] == value:  # 겅색 성공
                c += 1                  # 들어 있음
        return c

    def __cotains__(self, value: Any) -> bool:
        '''큐에 value가 있는지 판단'''
